...
**Description:** This is the data collected from the SSB 2015 cost survey. The costs collected at the vessel or business-level which will be described in the notes section. Note that variable names may change over time such that OFFICE_EXP could be for all vessels in one year and the selected vessel in another. All values are presented for the year in which they were queried (nominal dollars) and are reported at the vessel-level unless otherwise noted. The primary key is HULLNUM and SURVEY_YEAR (not VESSEL_NAME).  
...
**Description:** This is the data collected from the SSB 2022 cost survey. The costs collected at the vessel or business-level which will be described in the notes section. Note that variable names may change over time such that OFFICE_EXP could be for all vessels in one year and the selected vessel in another. All values are presented for the year in which they were queried (nominal dollars) and are reported at the vessel-level unless otherwise noted. The primary key is HULLNUM and SURVEY_YEAR (not VESSEL_NAME).  
 


```{r chunk1-5, echo = FALSE, warning = FALSE, message = FALSE }
# library(dplyr)
# library(dbplyr)
# library(readxl)
# library(tidyverse)
...
VCS_COST2022 <- read_excel(here::here("data/ORACLE_METADATA.xlsx"), sheet = "VCS_COST2022") 

cost2022<-VCS_COST2022
#COMVAR %>%
  kbl(cost2022, "html", caption ='VCS_COST2022 METADATA') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", full_width=F), 
               latex_options = c("repeat_header","scale_down"), font_size = 12) %>%
  scroll_box(width = "700px", height = "400px")
```
